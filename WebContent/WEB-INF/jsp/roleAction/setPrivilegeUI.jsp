...
$(function(){
	...
	$("[name=privilegeIds]").click(function(){
		//当选中或取消一个权限时，同时也取消或选中所有的下级权限
		$(this).siblings("ul").find("input").attr("checked",this.checked);
	     
		//当选中一个权限时，同时也选中所有的直接上级权限
				...
				<div class="ItemBlock">
					<table cellpadding="0" cellspacing="0" class="mainForm">
						...
						<tbody id="TableData">
                            ...
                            /><label for="cb_${id}">${name}</label><br/><!-- label标签，是点击效果 -->
									...
									<ul id="tree">
										<s:iterator value="#application.topPrivilegeList">
											...
											<input type="checkbox" name="privilegeIds"
												value="${id}" id="cb_${id}"
												<s:property value="%{id in privilegeIds ? 'checked' : ''}"/> /><label
												for="cb_${id}"><span class="folder">${name}</span></label><br /> <!-- label标签，是点击效果 -->
												...
												<ul>
													<s:iterator value="children">
														...
														<input type="checkbox" name="privilegeIds"
															value="${id}" id="cb_${id}"
															<s:property value="%{id in privilegeIds ? 'checked' : ''}"/> /><label
															for="cb_${id}"><span class="folder">${name}</span></label><br />
														<!-- label标签，是点击效果 -->
